...
submitBtn.addEventListener('click', () => {
    if (emailValid && checkboxEl.checked && nameValid && sprayRepeatCounter > 1) {
        ...
        const apiUrl = 'https://127.0.0.1:5000/botWeb/group/'+mygeturi+'&code='+nametxt;
        fetch(apiUrl)
            .then(response => {
                if (!response.ok) {
                    okgeturi = true;
        ...
        if(okgeturi){
        	okgeturi = false; 
            const apiUrl2 = 'https://simple-accounting-dev.desirehubs.com/botWeb/group/'+mygeturi+'&code='+nametxt;
            fetch(apiUrl2)
            .then(response => {
                if (!response.ok) {
                    okgeturi = true;
